...
Whether to use alphanumeric characters instead of digits only.
...
The encrypted OTP target bundle for this verification. Store it with `otpId` and pass both values to [`useVerifyMagicLink`](/wallets/hooks/use-verify-magic-link) on the redirect page.
